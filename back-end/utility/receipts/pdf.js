  ...
  try {
    ...
    const [[booking]] = await db.query(`
      SELECT 
        ...
        r.referral_name, r.referral_mobileno, r.referral_email,
    ...
    return new Promise((resolve, reject) => {
      ...
      // ================= HEADER =================
      // Left logo
 const headerY = 50; // starting Y position of the header
const logoWidth = 110;
const logoHeight = 120;

      // Draw logos if exists
if (fs.existsSync(logoPath)) {
  doc.image(logoPath, 420, headerY, { width: logoWidth, height: logoHeight }); // Right logo
}
      // Center text
      doc.font(fontEnglishBold).fontSize(22).fillColor("#000910")
        .text("KSN FUNCTION HALL", 1, 50, { align: "center" });
      doc.font(fontEnglish).fontSize(10).fillColor("#444")
        .text(
          "KSN Plaza, Opp. Rangaraju Rice Mills,\nPalakollu–Bhimavaram Main Road, Poolapalli,\nAndhra Pradesh – 534250\n  9845804760",
          { align: "center" }
        );
      doc.font(fontEnglish).fontSize(16).fillColor("#555")
        .text("Order / Payment Receipt", { align: "center" });

      doc.moveDown(1.5);

      const invoiceNo = `KSN-ORDER-${booking.booking_id}`;
      doc.font(fontEnglish).fontSize(10).fillColor("#000");
      doc.text(`Order No: ${invoiceNo}`, 50, doc.y, { continued: true });
      doc.text(`Order Date: ${new Date().toLocaleDateString("en-IN")}`, { align: "right" });
      doc.moveDown(0.5);
